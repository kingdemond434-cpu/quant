...
| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-07-31-venue and book NAV are com)` | claims `venue $6,227 (=1.25x futures start) vs book $18,670 (=1.24x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-01-venue and book NAV are com)` | claims `venue $6,227 (=1.25x futures start) vs book $18,811 (=1.25x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-02-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $13,296 (=0.89x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |
| CV | **health all_ok is consistent with organ logs** `(CV-2026-08-02-health all_ok is consisten)` | claims `all_ok=True, organs_ok=True` but the source says `34 stub logs vs 6 real logs in last 48h` -- a green dashboard while organs are dead means silent research outage | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-05-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $13,152 (=0.88x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-06-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,906 (=1.19x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-07-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,931 (=1.20x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-10-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,947 (=1.20x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-11-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,973 (=1.20x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |
| CV | **axis_shadows.json freshness claim is true** `(CV-2026-08-11-axis_shadows.json freshnes)` | claims `updated 2026-08-10 03:00` but the source says `age 24.5h, mtime drift 0.0h` -- a stale artifact presented as current is read as live state | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-12-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,944 (=1.20x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-13-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,732 (=1.18x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-14-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,806 (=1.19x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **venue and book NAV are compared on the SAME scope** `(CV-2026-08-15-venue and book NAV are com)` | claims `venue $6,070 (=0.58x futures start) vs book $17,908 (=1.19x total start)` but the source says `venue=FUTURES scope, portfolio=TOTAL scope -- not comparable` -- run_venue_divergence_shadow logs pct_diff between these and that series is what calibrates the GAP #19 breaker; calibrating on a scope mismatch produces a breaker that never fires or always fires. Fix the shadow to compare like-for-like BEFORE arming anything from it. | BEFORE any live capital |

| CV | **axis_shadows.json freshness claim is true** `(CV-2026-08-28-axis_shadows.json freshnes)` | claims `updated 2026-08-20 03:59` but the source says `age 192.8h, mtime drift 0.0h` -- a stale artifact presented as current is read as live state | BEFORE any live capital |
