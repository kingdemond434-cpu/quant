...
### WS-009 era-venue public tape is DISPLAY-ROUNDED and book-truncated — a provenance prior for every archived pre-2017 tape   [observations: 2]
...
latest: 2026-08-19 RU s3 continuation · SECOND ERA VENUE, at the FEE-METADATA layer: forum.btcsec.com
topic 6549 (1b bot lite vendor changelog, v1.0.0.47, 2014-06-11) — "Cryptsy прошито новое fee 0.25%
(их API по прежнему возвращает 0.2/0.3)": Cryptsy raised its fee to 0.25% and its own fee ENDPOINT
kept returning 0.2/0.3, so the vendor HARDCODED the true fee against the venue's own API. An era cost
model built from the venue's published fee metadata understates costs — the defect family extends
past the tape (price/size precision, WS-009 obs 1) into the venue's cost-parameter API. Same thread,
vendor's own marketing corroborates the taker-dominance leg from the seller side: the bot is built on
FOK ("Fill or Kill — мгновенная торговля... позволяет не замораживать финансовые средства") — but
that corroboration is SAME-VENDOR as topic 4382 (1bbot), so it widens the evidence, not the source count.
...
GOC.IO/Cex.IO/Bitstamp/BtcChina/Mintpal/Bittrex by Jul-2014, "145 валютных пар") — (a) the vendor
itself DISTRIBUTES user-contributed strategy files from its forum posts (Claxxon's "Crypto Miner"
strategy "заточенная под спад" — buy-cheap-on-decline, the grid class's add-inventory-down economics
shipped as a file; toenail's "ПОНЧИК"), so the monoculture channel is vendor-CURATED, not just
vendor-enabled; (b) licence is PER API KEY ($99.95/key) and the May-2014 promo required buyers to
"Прислать нам на почту API-ключи" (email us your API keys) — the vendor holds a REGISTRY of customer
...
honest qualification (stated so a later reader does not over-read this): the era evidence is INTRADAY and the desk's premium series is DAILY, so an outage contaminates a daily close only when it spans the sampling instant — the daily exposure is strictly weaker than the era anecdote implies. This is a prior to MEASURE, never one to assume forward.

**OBSERVATION 2 — MEASURED, 2025, AND IT RETIRES THIS ENTRY'S OWN "2017-18 IS NOT 2026" CAVEAT (KR frontier miner s3, 2026-08-13).** The era half of this WS rested on one 2018 anecdote, and the caveat above correctly refused to carry it forward. It no longer has to. Measured directly off both venues' 1-minute tapes: **Bithumb KRW-BTC has a 10.50-hour HOLE — last bar 2025-03-23T15:30:00Z (00:30 KST 03-24), next bar 2025-03-24T02:00:00Z (11:00 KST), price +51bp across the halt — while Upbit's tape runs CONTINUOUS through the identical window** (checked bar-by-bar; the two align to the minute on either side of the gap, so the gap is Bithumb's and not an artifact of the comparison). **2025-03-24 is the date Bithumb migrated its real-name deposit account from NH Nonghyup to KB Kookmin** — i.e. the outage is a *banking-rail cutover window*, which is the strongest possible form of this WS's central claim: the venue that halts is the venue with the rail event, so tape liveness is **correlated with the treatment** rather than independent of it, exactly as the `direction` field argues. Reproduce: `GET https://api.bithumb.com/v1/candles/minutes/1?market=KRW-BTC&count=200&to=2025-03-24T13:00:00` — **note `to` is KST-interpreted on Bithumb and UTC-interpreted on Upbit (OP-090, minted as OP-072 on the s3 branch and renumbered at landing); getting that wrong shifts the window 9h and hides the gap.** Full record: `data/kr_venue_bank_rail.json`.
**What observation 2 changes, precisely:** it does NOT make this tradeable and does not touch the promotion-check below. It moves the prior from *"an era board says venues froze"* (2018, anecdotal, plausibly obsolete) to *"a big-2 KR venue went dark for 10.5h on a scheduled rail event 5 months ago and the desk's own instrument would have booked the gap as a spread"* (2025, measured, current). The daily-vs-intraday qualification still stands in full — a 10.5h gap spanning 15:30→02:00 UTC **does** straddle Bithumb's KST-day close, so this particular instance would have hit a daily bar, but one instance is not a rate and no rate has been measured.
...
### WS-012 the sub-daily tape screen tests proprietary microstructure features against a COMMON-FACTOR-dominated target, and the residualised cell has never been run   [observations: 1 external + 2 internal reads]
first-seen: 2026-08-13 EN frontier miner s H, opening the Kaggle competition ground (Records family, never touched by any seat since first named 07-25).
external observation (primary source, archived, verified this session): **G-Research** — a top-tier quant research firm designing a public crypto forecasting task with $125,000 of its own money and every incentive to make the task winnable — defined its target as **"`Target` - 15 minute residualized returns"** (Kaggle competition dataset description, `databundleVersionId` 3147400, recovered from the 2021-11-03 Wayback capture of `/c/g-research-crypto-forecasting/data` via OP-068). Independently corroborated on the shape: the residual is taken against a market return built as **the weighted sum of the 14 assets' returns**, per-asset weights shipped in `asset_details.csv`. Competition scale: **1,946 teams / 2,398 competitors / 3,141 entries**, models **frozen 2022-02-01** and scored on **live forward data to 2022-05-03**.
internal reads (this session, from the artifacts, not from context):
  - `scripts/screen_moat.py:317` — the sub-daily tape screen loops **per symbol** and builds `ret = period_returns(rows, fts)`: a **per-symbol RAW forward return** off that symbol's own trade tape, at `HORIZONS_S` spanning 60s–900s. **No cross-sectional demeaning, no beta residualisation, at any horizon.**
  - `libs/research/panel_breadth.py:35-42` — the desk's OWN measurement on its own 139-symbol panel: **raw returns rho +0.5288 → 1.88 effective bets; relative (demeaned) returns rho -0.0069 → 139**. The module states "the screen's target IS the cross-sectionally demeaned return" — **true of the daily PANEL screen, and not true of the tape screen.** The two layers disagree about target construction and nothing compares them (the L1.61 class: each artifact individually correct, the contradiction living only in the relation).
direction: a raw 60s–900s forward return is dominated by the common (BTC-beta) factor, so a microstructure feature's IC against it is largely a measurement of *"does this feature predict the market move"* — the most crowded and most competed quantity available, and one that is **common across symbols**, so K per-symbol survivors would be close to ONE discovery rather than K. The residual isolates the symbol-specific component, which is (a) where the desk's own breadth arithmetic says the independent information concentrates, (b) where book-pressure / trade-intensity / replenishment features have a mechanism story at all, and (c) what the one large, well-funded, forward-validated public experiment at this exact horizon chose. **This is a MISSING CELL, not a defect** — a raw per-symbol return is the quantity actually traded, so screening it is legitimate; what is missing is that the residualised construction has never been screened, which the TARGET/HORIZON SWEEP DUTY makes an un-swept cell rather than a settled choice.
honest qualifications (stated so a later reader cannot over-read this):
  - The 1.88-vs-139 breadth arithmetic is about **pooling** K symbols into one statistic. `screen_moat` screens symbols **separately**, so that arithmetic does **not** mean its per-cell t-stats are inflated. It bears on how many INDEPENDENT discoveries a set of per-symbol survivors represents, not on whether any one cell is correctly powered.
  - Cross-sectional demeaning is beta-residualisation with beta forced to 1. G-Research estimated per-asset betas. These are different constructions and the cheap version of this test is the demeaned one.
