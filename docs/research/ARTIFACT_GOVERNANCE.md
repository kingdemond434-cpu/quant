...
| `ELITE_QUANT_INTELLIGENCE_MANDATE.md` | **DOCTRINE** | Standing principal directive on elite-firm capability recovery, extreme-outlier forensics and future-frontier search (2026-08-13, Parts I-II, three builder seats), extended 2026-08-14 with **PART III — EXTERNAL QUANT INTELLIGENCE MINING** across the English and Chinese research ecosystem, which binds EVERY brain, miner, LLM seat and collector that reads an outside source. Part III's hard limits: the access boundary is legal and absolute (public or authorized only, no bypass of any control); social proof is never evidence and may order the queue but never decide acceptance; a source earns its budget by MEASURED downstream conversion to live economic value, never by size or popularity; and a miner has no promotion authority at all. Stored VERBATIM rather than summarised: a policy paraphrase drifts from the policy while both look compliant, the same reason miner briefs carry their mandates in full. Changes by principal decision only, never on a schedule — a cadence floor here would imply the law expires. |
...
| `docs/research/mt5_source_seeds.md` | **LIVING** | Claimed by L1.52 (information mining is permanently active) and by the miners' own anti-breadth-theater rule. It is deliberately NOT the catalogue: the catalogue (`data_axis_watchlist.md`) carries graded cards that owe verification decisions, and at 8 pending of 18 the desk's measured bottleneck is verification, not cataloguing. A seed map carries no verification debt, so it can hold hundreds of grounds without making that bottleneck worse — and a source only becomes a card by producing something. Grows as the discovery layer finds grounds absent from it; the list is seeds, never a ceiling. **REPLACED 2026-09-05:** the predecessor (`crypto_source_seeds.md`, later `offbook_source_seeds.md`) mapped a Binance/crypto-native universe and was deleted under the MT5 universe mandate (2026-08-18). The structure — a reading map with no verification debt — is what carried over; the grounds did not. | n/a |

### Added 2026-08-09 (survivor-yield / E[log W] expansion)
...
| `docs/LAWS.md` + `docs/RESEARCH.md` | **DOCTRINE** | The two operative documents of the 2026-08-25 consolidation: LAWS binds every seat, RESEARCH binds every research organ (each dig's first standing order). Amendment is L2.8 evidence-gated and never toward conservatism; no cadence refreshes them. `check_doctrine_diff.py` surfaces every edit as a principal act. | never |
| `docs/MANDATE_COVERAGE.md` | **TERMINAL** | The disposition map of the consolidation — which annex went where, verified zero-regression at consolidation time. It records one decision at one moment; a later re-consolidation produces a NEW map, never an edit to this one. | never |
| `docs/policy/DEEPSEEK_SECOND_FLYWHEEL_MANDATE.md` | **DOCTRINE** | Principal standing mandate for the DeepSeek donation-only flywheel (RESEARCH §5 codifies its authority limits: donates findings, refused promotion/allocation/policy/code-merge authority in code). Changes by principal decision only. | never |
| `docs/research/MOAT_NODE_SPEC.md` | **DOCTRINE** | Standing spec for moat recorder nodes (what a node must record, stamp and replicate). Implementation changes ship as code against the spec; the spec itself moves only by decision. | never |
