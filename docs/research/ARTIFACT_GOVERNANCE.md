...
| `docs/research/offbook_source_seeds.md` | **LIVING** | Claimed by L1.52 (information mining is permanently active) and by the miners' own anti-breadth-theater rule. It is deliberately NOT the catalogue: the catalogue (`data_axis_watchlist.md`) carries graded cards that owe verification decisions, and at 8 pending of 18 the desk's measured bottleneck is verification, not cataloguing. A seed map carries no verification debt, so it can hold 450 grounds without making that bottleneck worse — and a source only becomes a card by producing something. Grows as `kimi_hunter` discovers grounds absent from it; the list is seeds, never a ceiling. | n/a |
