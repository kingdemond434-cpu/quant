...
**Universal edge capture** — every lawfully discovered novel testable edge receives canonical
status: IMMEDIATE_TEST, NEXT_BATCH_TEST, BLOCKED_DATA, BLOCKED_IMPLEMENTATION, DEDUPLICATED,
FORMALLY_DOMINATED, NON_TESTABLE or EMPIRICALLY_REJECTED. No silent dismissal based on prestige.

### 47-59. Learning, self-improvement, economics

RL only where justified; define precise reward, avoid raw-PnL reward hacking, compare against
simpler supervised/ranking/policy alternatives, keep only if it wins controlled tests. Persist
complete research trajectories (question, evidence, hypotheses, code, errors, experiments,
results, decisions, eventual OOS/live outcomes) and use high-quality ones to improve retrieval,
prompts, planning, tool choice, stopping, triage and ranking — never learning blindly from final
PnL.

Failure science: study failed elite-firm ideas, failed AI-native systems, former top performers,
retracted claims, alpha decay, live/backtest gaps, agent failures, hallucinations and execution
failures. Negative results must alter future behaviour; never ban permanently on prior failure
alone when new evidence materially changes the hypothesis.

Bottleneck law: continuously measure DISCOVERY, SPECIFICATION, IMPLEMENTATION, EXPERIMENT,
VALIDATION, SURVIVOR and PORTFOLIO CONVERSION rates. The current bottleneck gets engineering
priority; do not stop unrelated healthy factory components while fixing it.

Checkpoint everything expensive; cache reusable research; measure VALIDATED_INFORMATION / COMPUTE,
DECISIVE_EXPERIMENTS / COMPUTE and SURVIVOR_VALUE / COMPUTE. Every candidate research action
competes on expected economic information value × probability of decisive result × portfolio
relevance ÷ total marginal cost. Reduce cadence for low-yield sources; preserve cheap exploration.
No dedicated high-frequency token burner — use change detection, PIT hashes, feeds, search deltas
and event-triggered refresh.

### 60-90. Portfolio, stress, auditability

Every promising candidate must survive realistic fees, spread, slippage, funding, borrow, impact,
venue constraints, latency and liquidity — alpha that cannot be captured is not alpha for this
desk. Evaluate survivors on standalone edge, confidence, persistence, independence, tail,
capacity, capital displacement and marginal E[log W]; maximise the economically useful survivor
...
code version, data version, model/harness version and verdict. If an improvement cannot later be
reconstructed, it is not canonical knowledge.

### The extreme-outlier hunter

EXTREME OUTLIER → VERIFY → EXPLAIN → ISOLATE REAL EDGE → COMPARE AGAINST OUR QUANT → RECOVER
MISSING CAPABILITY → BUILD INDEPENDENT CHALLENGER → CANONICAL TEST → DESCENDANTS → SURVIVOR /
NEGATIVE MEMORY → PORTFOLIO CONSEQUENCE → RESEARCH-SYSTEM IMPROVEMENT.

A headline such as "$1 → $3M" enters as EXTREME_CLAIM_UNVERIFIED, never as fact. Required fields:
STARTING_CAPITAL, EXTERNAL_DEPOSITS, WITHDRAWALS, REALIZED_PNL, UNREALIZED_PNL, CURRENT_EQUITY,
DATES, TRADE_COUNT, POSITION_SIZES, LEVERAGE, MARKETS, FEES, FUNDING, GAS, SLIPPAGE, TURNOVER,
MAX_DRAWDOWN, BIGGEST_WIN, BIGGEST_LOSS, WIN_RATE, PAYOFF_RATIO, TAIL_RISK, CAPITAL_PATH,
KNOWN_FAILURES, SOURCE_PROVENANCE. Capital-path forensics: INITIAL + DEPOSITS − WITHDRAWALS +
REALIZED + OPEN = CURRENT. A $1 deposit followed by a $500,000 transfer is not "$1 → $500,001".

Bot attribution is BOT / HUMAN / HUMAN+AI / COPY TRADER / MARKET MAKER / SCRIPT / UNKNOWN, and
uncertain stays UNKNOWN. Mechanism recovery decomposes into latency arb, cross-venue, cross-market,
resolution-latency, oracle lag, market making, spread capture, rebates, forced flow, liquidation,
funding, basis, information speed, event ingestion, microstructure, queue position, copy-trading
selection, statistical forecasting, regime filter, bet sizing, netting, tail selling, martingale,
leverage, concentration, LUCK, unverifiable information, or MEASUREMENT ARTIFACT. Do not assume
the glamorous explanation.

Trade-level replay estimates EDGE_AT_DECISION / AT_FILL / +100ms / +1s / +5s / +30s / +1m where
data permits. Estimate copyability decay, and never assume historical extraordinary returns are
reproducible now. Decompose observed return into genuine alpha + beta + leverage + concentration +
tail + liquidity premium + capacity effect + luck + measurement error; do not recreate extreme
return by recreating extreme risk. For every spectacular winner search the MATCHED FAILURE COHORT
and keep tracking DISAPPEARED WINNERS. Estimate risk-of-ruin counterfactuals and bootstrap
alternative orderings: was the observed path typical of the edge or an extreme right-tail
realisation? Detect fraud/error patterns (impossible liquidity, inconsistent returns, hidden
deposits, fake screenshots, simulated results presented as live, paper PnL as realised) and record
them as NEGATIVE INTELLIGENCE. Decompose AI contribution: AI BUILT TOOL vs AI GENERATED EDGE vs AI
AUTONOMOUSLY TRADED — this distinction is mandatory.

### Research environments, evolution and governance

Hidden quant OOS environments (agents research development data; final score from sealed future
data), anonymised market environments, a research-depth metric penalising shallow idea hopping, a
VALIDATION-REUSE BUDGET (as reuse grows, effective holdout value falls — force rotation),
INCREMENTAL ABLATION (a feature survives on incremental model/portfolio value, never on IC > 0 or
low correlation alone), tool-quality decay benchmarks, drawdown-behaviour evaluation, and
progressive difficulty as the champion improves.

Frozen research artifact: HYPOTHESIS, ECONOMIC RATIONALE, EXECUTABLE IMPLEMENTATION, DATA,
PARAMETERS, EXPERIMENT EVIDENCE, REJECTED ALTERNATIVES, REVIEW, LINEAGE, STATUS. Fresh-context
adversarial re-execution: researcher finishes → artifact freezes → independent reviewer starts
fresh, reads raw evidence, re-executes critical tests, looks specifically for rejection reasons,
may VETO. VERIFIED FABRICATION = ZERO CREDIT: a claim that a test ran when it did not, metrics
disagreeing with the artifact, or wrong candidate/evidence mapping voids the lineage's evidence
credit until repaired.

Pending-aware allocation, backpressure, successive-halving tournaments with a FALSE-NEGATIVE
SAFEGUARD (a noisy early sample must not permanently kill a high-upside edge; allow resurrection),
external evaluators the researcher cannot manipulate, prompt economy (every model invocation must
repay its cost), and minimal stable roles rather than agent bureaucracy. Specialists start at LOW
AUTHORITY and are PROMOTED / MUTATED / HIBERNATED / DELETED on measured incremental value.

### Future frontier

The current frontier is not the destination. Search the CATEGORY frontier and beyond it; harvest
cross-domain advances (scientific agents, theorem proving, causal discovery, planning, world
models, active learning, Bayesian experimental design, program synthesis, formal verification,
continual learning) BEFORE waiting for another fund to apply them. Every research process is
mutable and challengeable. Every capability is eligible for retirement: CHAMPION → MATURE →
CHALLENGED → SUPERSEDED → HIBERNATED → RETIRED. No technology religion — the criterion is whether
it improves the economic research function.

Track CAPABILITY DISCOVERY REGRET (public availability → discovery → test → decision →
assimilation) and run missed-frontier postmortems. Internal failures generate external research
questions. Maintain a nonzero exploration budget forever and a nonzero exploitation budget forever.
Compare CURRENT US against PAST US, not only against competitors. Track EXTERNALLY_INSPIRED vs
INTERNALLY_ORIGINATED capabilities; a rising share of the latter is evidence the factory itself is
improving.

### Terminal law

MINE WHAT THE BEST FIRMS AND NEW AI-NATIVE LABS REVEAL. DO NOT BELIEVE THEIR MARKETING. SEPARATE
DIRECT EVIDENCE FROM INFERENCE. RECONSTRUCT ECONOMIC FUNCTION, NOT PRIVATE IP. TURN EVERY MATERIAL
CAPABILITY CLAIM INTO A TESTABLE GAP. TURN EVERY TESTABLE GAP INTO A SOLO-SCALE CHALLENGER WHEN
VOI JUSTIFIES IT. BENCHMARK AGAINST THE CURRENT DESK. KEEP ONLY WHAT REALITY PROVES BETTER. USE
THEIR FAILURES AS AGGRESSIVELY AS THEIR SUCCESSES. MAKE RESEARCH MEMORY COMPOUND. MAKE OUR OWN
EXECUTION HISTORY PROPRIETARY DATA. MAKE FAILURE LEARNED, NOT FORGOTTEN.
