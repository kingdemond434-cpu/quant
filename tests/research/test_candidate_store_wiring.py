...
queue reported `n_candidates: 0` every six hours, the collapse detector reported PERFECT diversity
...
@pytest.mark.parametrize(
    ("module", "name"),
    [(run_promotion_queue, "_DB"),
     (run_generation_diversity, "_DB"),
     (max_audit, "_CANDIDATE_DB")],
)
def test_reader_does_not_point_at_the_phantom_path(module, name):
    ...
    assert path.name == "sor_research.sqlite"
