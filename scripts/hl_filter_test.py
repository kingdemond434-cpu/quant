...
N_TRY=1800; MIN_PTS=60; MIN_SPAN=200
...
if CACHE.exists():
    recs=json.loads(CACHE.read_text()); print(f"cached cohort {len(recs)}")
else:
    ...
    CACHE.write_text(json.dumps(recs)); print(f"cohort {len(recs)} cached")
...
def show(name,m):
    if m.sum()<5: print(f"  {name:<44} n={int(m.sum()):5d}  (too few)"); return
    s=hr[m]
    print(f"  {name:<44} n={int(m.sum()):5d} | fwd mean {s.mean()*100:+7.1f}% | "
...
q=np.quantile(fr,0.90); show(f"top 10% by past return (>{q*100:.0f}%)", fr>=q)
q=np.quantile(fr,0.99); show(f"top  1% by past return (>{q*100:.0f}%)", fr>=q)
print()
...
show("ELITE STRICT: prof + Sh>2 + cons>60% + dd>-15%", (fr>0)&(fs>2)&(fc>0.60)&(fd>-0.15))
