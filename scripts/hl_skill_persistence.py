...
def _decile_spread(form: np.ndarray, hold: np.ndarray, q: int = 10):
    ...
    return {
        "top_decile_mean_ret": round(float(top.mean()), 5),
...
def _run(form: np.ndarray, hold: np.ndarray, label: str) -> dict:
    ...
    out = {
        ...
        "t_stat": round(t, 2),
        "null_rho_p95": round(float(np.percentile(null, 95)), 4),
        "decile": dec,
    }
    print(f"\n[{label}] n={len(form)}")
    print(
        f"  rank-corr(formation, holding) rho={rho:+.4f}  t={t:+.2f}  "
        f"(shuffled-null |rho| p95={np.percentile(null, 95):.4f})"
    )
    if dec:
        print(
            f"  top-decile fwd ret {dec['top_decile_mean_ret']:+.4f} vs bottom "
            f"{dec['bottom_decile_mean_ret']:+.4f} | spread {dec['spread']:+.4f} (t "
            f"{dec['t']:+.2f})"
        )
    return out
...
def main() -> None:
    ...
    for r in rows:
        try:
            ...
            continue
    print(
        f"after filters (acctValue>=${MIN_ACCT_VALUE:,.0f}, monthVlm>=${MIN_MONTH_VLM:,.0f}): "
        f"{len(recs)} traders"
    )
    if len(recs) < 200:
    ...
    lo, hi = np.percentile(form, [0.5, 99.5])
    form_w = np.clip(form, lo, hi)
    lo2, hi2 = np.percentile(hold, [0.5, 99.5])
    hold_w = np.clip(hold, lo2, hi2)
        ...
        json.dumps(
            {
                ...
                "verdict": verdict,
                "results": results,
                "caveat": "leaderboard is a current snapshot; fully-closed blowups may be "
                "absent -> "
