...
def h2_retention(res):
    ...
    try:
        url = subprocess.check_output(
            ["git", "remote", "get-url", "origin"], text=True, cwd=str(ROOT)
        ).strip()
    ...
    UNIV = [
        ...
        ("AVAXUSDT", "ava-labs/avalanchego"),
        ("ATOMUSDT", "cosmos/cosmos-sdk"),
        ("DOTUSDT", "paritytech/polkadot-sdk"),
        ("NEARUSDT", "near/nearcore"),
        ("FILUSDT", "filecoin-project/lotus"),
        ("ALGOUSDT", "algorand/go-algorand"),
        ("EOSUSDT", "AntelopeIO/leap"),
        ("ZILUSDT", "Zilliqa/zilliqa"),
        ("ONEUSDT", "harmony-one/harmony"),
        ("FTMUSDT", "Fantom-foundation/go-opera"),
        ("XTZUSDT", "tezos/tezos"),
        ("EGLDUSDT", "multiversx/mx-chain-go"),
        ("KAVAUSDT", "Kava-Labs/kava"),
        ("NEOUSDT", "neo-project/neo"),
        ("QTUMUSDT", "qtumproject/qtum"),
        ("ONTUSDT", "ontio/ontology"),
    ]
    ...
    def authors(repo, since, until):
        ...
        while page <= 3:
            u = (
                f"https://api.github.com/repos/{repo}/commits?since={since}&until={until}"
    ...
    w = [
        (now.replace(day=1) - __import__("datetime").timedelta(days=d)).strftime(
            "%Y-%m-%dT00:00:00Z"
        )
        for d in (180, 90, 0)
    ]
    ...
    print(
        f"H2 CONTRIBUTOR RETENTION  n={len(use)}  retention {ret.min():.2f}-{ret.max():.2f}  "
        f"rho {rho:+.3f} (t {t:+.2f})  top-bottom rel-return {spread * 100:+.1f}%  "
        f"{'PASS' if abs(t) >= BAR else 'fail'}"
    )
    res.append(
        {
            "h": "H2_contributor_retention",
            "n": len(use),
            "rho": round(rho, 4),
            "t": round(t, 2),
...
def h3_flow_vol(res):
    ...
    print(
        f"H3 FLOW -> REALISED VOL  n={len(zv)}  rho {rho:+.3f} (t {t_eff:+.2f} overlap-adj)  "
        f"hi-flow vol {hi * 100:.2f}% vs lo {lo * 100:.2f}% ({hi / max(lo, 1e-9):.2f}x)  "
        f"{'PASS' if abs(t_eff) >= BAR else 'fail'}"
    )
    res.append(
        {
            "h": "H3_flow_to_volatility",
            "n": len(zv),
            "rho": round(rho, 4),
            "t_overlap_adj": round(float(t_eff), 2),
            "vol_ratio": round(hi / max(lo, 1e-9), 3),
...
def h4_accel(res):
    ...
    for h in (7, 14, 30):
        ...
        t_eff = t / np.sqrt(h)
        print(
            f"H4 ATTENTION ACCELERATION {h:>2}d  n={int(m.sum())}  rho {rho:+.3f} "
    ...
    res.append(
        {
            "h": "H4_attention_acceleration",
            "best_horizon_d": best[0],
            "t_overlap_adj": round(float(best[1]), 2),
            "rho": round(float(best[2]), 4),
...
def main() -> None:
    ...
    OUT.write_text(
        json.dumps(
            {"updated": datetime.now(tz=UTC).isoformat(), "bar_t": BAR, "results": res}, indent=1
        ),
        "utf-8",
    )
    print(f"  -> {OUT}")
