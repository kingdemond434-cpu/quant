...
N=260; BK=4*3600_000; COINS=["BTC","ETH","SOL"]
...
sel=cand[:N]
print(f"leaderboard {len(rows)} -> directional cohort {len(sel)} "
...
for i,(av,a,turn) in enumerate(sel):
    ...
    if (i+1)%80==0: print(f"  {i+1}/{len(sel)} ok={ok}",flush=True)
...
for c in COINS:
    ...
    if len(grid)<60:
        print(f"{c}: thin ({len(bks)} bkts, {len(grid)} aligned)"); continue
    sig=np.array([flow[c][b] for b in grid]); cl=np.array([px[b] for b in grid])
    ret=np.zeros(len(cl)); ret[1:]=cl[1:]/cl[:-1]-1
    r=stage_a_screen(sig,ret,name=f"hl_dir_flow_{c}",zwin=20); r["coin"]=c
    res.append(r); pool.append(r.get("ic",0.0))
...
if len(pool)>1:
    p=np.array(pool); t=float(p.mean()/(p.std()/np.sqrt(len(p)))) if p.std() else 0.0
    print(f"\nPOOLED mean IC {p.mean():+.4f} (t {t:+.2f}, n={len(p)}) | bar |t|>=2.7")
Path("data/hl_dir_flow.json").write_text(json.dumps(
