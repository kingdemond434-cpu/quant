...
_STEPS = [
    ...
    ("walcl_clock", "scripts/derive_walcl_clock.py", 60),  # R0031 pre-registered forward clock
    ...
    (
        ...
        90,
    ),  # failure patterns + transfer pipeline + novelty + anomalies
    (
        "research_cio",
        "scripts/research_cio.py",
        90,
    ),  # info advantage + blind spots + north star + scheduler
    ...
    (
        ...
        90,
    ),  # harvest experiments -> permanent objects
    (
        "desk_brief",
        "scripts/research_exchange.py brief",
        60,
    ),  # daily research board / external-LLM brief
    (
        "contributor_score",
        "scripts/research_exchange.py score",
        60,
    ),  # which intelligence source earns allocation
...
def main() -> None:
    ...
    print(
        f"CRO cycle {entry['date']}: next-ROI={entry['next_highest_roi_task']} "
        f"| constraint={entry['binding_constraint']}"
    )
    if nxt is None:
        print(
            "  no positive-ROI engineering task -> research capital = WAIT on data clocks "
