...
N=320; BK=4*3600_000
COINS=["BTC","ETH","SOL","DOGE","AVAX","LINK","ARB","OP","SUI","APT","LTC","XRP","BNB","NEAR","TIA","INJ"]
...
for r in rows:
    try:
        ...
        turn=vlm/av
        if 1.0<=turn<=25.0: cand.append((av,a))
    except (TypeError,ValueError): continue
cand.sort(reverse=True); sel=cand[:N]
...
for i,(_av,a) in enumerate(sel):
    ...
    for f in fills:
        c=f.get("coin")
        if c not in flow: continue
        try: t=int(f["time"]); px=float(f["px"]); sz=float(f["sz"])
        except (KeyError,TypeError,ValueError): continue
        flow[c][t//BK]+= (1.0 if str(f.get("side","")).upper().startswith("B") else -1.0)*px*sz
    if (i+1)%100==0: print(f"  {i+1}/{len(sel)} ok={ok}",flush=True)
print(f"with fills {ok}/{len(sel)}",flush=True)

res=[];ics=[];sprd=[]
for c in COINS:
    bks=sorted(flow[c])
    if len(bks)<70: continue
    try:
        kl=json.loads(_get(f"https://api.binance.com/api/v3/klines?symbol={c}USDT&interval=4h&limit=1000",40))
    except Exception: continue
    px={int(k[0])//BK:float(k[4]) for k in kl}
    grid=[b for b in bks if b in px and (b+1) in px]
    if len(grid)<70: continue
    sig=np.array([flow[c][b] for b in grid]); cl=np.array([px[b] for b in grid])
    ret=np.zeros(len(cl)); ret[1:]=cl[1:]/cl[:-1]-1
    ...
    for t in range(20,len(sig)):
        w=sig[t-20:t]; sd=w.std(); z[t]=(sig[t]-w.mean())/sd if sd>0 else 0
    zv=z[20:-1]; fv=np.roll(ret,-1)[20:-1]
    k=max(1,len(zv)//3); o=np.argsort(zv)
    sp=float(fv[o[-k:]].mean()-fv[o[:k]].mean())
    res.append({"coin":c,"n":len(grid),"ic":r.get("ic"),"same":r.get("same_period_corr"),
                "resid":r.get("residual_ic"),"spread":round(sp,5),"verdict":r["verdict"]})
    ics.append(r.get("ic",0.0)); sprd.append(sp)
    print(f"{c:5s} n={len(grid):4d} IC {r.get('ic'):+.4f} same {r.get('same_period_corr'):+.3f} "
          f"resid {r.get('resid') if 'resid' in r else r.get('residual_ic'):+.4f} "
          f"terc_spread {sp*100:+.3f}% | {r['verdict']}",flush=True)

if len(ics)>=3:
    a=np.array(ics); s=np.array(sprd)
    ...
    print(f"  mean tercile spread {s.mean()*100:+.3f}% (t {ts:+.2f}) | positive in {int((s>0).sum())}/{len(s)}")
    print(f"  BAR: |t|>=2.7 on BOTH + consistent sign -> {'PASS' if abs(ti)>=2.7 and abs(ts)>=2.7 else 'FAIL'}")
Path("data/hl_breadth_flow.json").write_text(json.dumps(
    {"updated":datetime.now(tz=UTC).isoformat(),"cohort":ok,"results":res},indent=1),"utf-8")
