...
def yahoo(sym: str, rng: str = "2y") -> dict[str, float]:
    ...
    return {
        ...
        for t, c in zip(r["timestamp"], q, strict=False)
        if c
    }
...
def main() -> None:
    ...
    print(
        "    constraint: US institutions legally CANNOT trade offshore perps (licence, not "
        "friction)\n"
    )
    cme = yahoo("BTC=F")
    ...
    for nm, x in (("CME basis", cb), ("offshore basis", ob), ("SPREAD (cme-offshore)", sp)):
        print(
            f"  {nm:<24} mean {x.mean() * 10000:+8.1f}bps   sd {x.std() * 10000:7.1f}bps   "
    ...
    print(
        f"  fraction of days |spread| > {ROUND_TRIP_BPS:.0f}bps round-trip cost: "
        f"{tradeable * 100:.0f}%"
    )
    ...
    verdict = (
        ...
        if (wide and reverts and bounded and tradeable > 0.5)
        else "TOO TIGHT (fails the >50bps width bar)"
        if not wide
        else "DRIFTS (no reversion)"
        if not reverts
        else "VIOLENT (unbounded)"
        if not bounded
        else "RARELY TRADEABLE (spread < costs most days)"
    )
    print(f"\n  VERDICT: {verdict}")
    print(
        f"    width>50bps {wide} | reverts<30d {reverts} | bounded {bounded} | "
