...
if not recs:
    ...
    sel = cand[LO:HI]
    print(
        f"OOS cohort: accounts ranked {LO}-{HI} by value ({len(sel)}) -- DISJOINT from in-sample",
        flush=True,
    )
...
if k >= 5:
    s = hr[m]
    pos = (s > 0).sum()
    sd = np.sqrt(k * base * (1 - base))
    z = (pos - k * base) / sd if sd > 0 else 0
    print(
        f"  n={k} | fwd mean {s.mean() * 100:+.1f}% | median {np.median(s) * 100:+.1f}% | "
        f"positive {pos}/{k} = {pos / k * 100:.0f}%"
    )
    print(f"  vs base rate {base * 100:.0f}%  -> binomial z {z:+.2f}")
    replicates = z >= 2.0 and s.mean() > hr.mean()
    print(
        "  VERDICT: "
        + ("REPLICATES (lift holds OOS)" if replicates
           else "FAILS TO REPLICATE (was specification search)")
    )
else:
    print(f"  n={k} -- too few to judge")
Path("data/hl_oos_elite.json").write_text(
    json.dumps(
        {
