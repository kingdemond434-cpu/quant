...
N_TRY = 420
MIN_PTS = 60  # need a real curve, not a few points
...
for i, (_av0, a) in enumerate(sel):
    ...
    if span < 200:
        continue  # need a LONG record (>~7 months)
    k = int(len(ret) * 0.6)
    f, h = ret[1:k], ret[k:]
...
if len(recs) < 40:
    raise SystemExit("insufficient long-history cohort")
...
hs = np.array([r["h_sharpe"] for r in recs])
out = {}
for nm, key in (
    ("formation SHARPE", "f_sharpe"),
    ("formation CONSISTENCY", "f_cons"),
    ("formation RETURN", "f_ret"),
    ("formation MAXDD (higher=safer)", "f_dd"),
):
    ...
    print(
        f"\n[{nm}]  -> holding RETURN rho {r1:+.3f} (t {t1:+.2f}) | "
        f"-> holding SHARPE rho {r2:+.3f} (t {t2:+.2f})"
    )
    print(
        f"   top-quartile holding ret {top.mean() * 100:+.1f}% "
        f"(median {np.median(top) * 100:+.1f}%) vs "
        f"bottom {bot.mean() * 100:+.1f}% (median {np.median(bot) * 100:+.1f}%)"
    )
    out[nm] = {
        "rho_ret": round(r1, 4),
        "t_ret": round(t1, 2),
        "rho_sharpe": round(r2, 4),
        "t_sharpe": round(t2, 2),
        "top_q_mean": round(float(top.mean()), 4),
        "bot_q_mean": round(float(bot.mean()), 4),
    }
print(
    f"\ncohort holding-period mean {hr.mean() * 100:+.1f}% median {np.median(hr) * 100:+.1f}% "
    f"| positive {int((hr > 0).sum())}/{len(hr)}"
)
Path("data/hl_longterm_skill.json").write_text(
    json.dumps(
        {
