...
def main() -> None:
    ...
    a = sub.add_parser("add"); a.add_argument("--model", required=True)
    a.add_argument("--summary", required=True); a.add_argument("--severity", default="med")
    a.add_argument("--ruling", default="accepted", choices=["accepted", "rejected"])
    a.set_defaults(fn=add)
    fx = sub.add_parser("fix"); fx.add_argument("--id", required=True)
    fx.add_argument("--commit", default=None)
    fx.set_defaults(fn=lambda x: _set(x, "fixed", {"fix_commit": x.commit}))
    v = sub.add_parser("verify"); v.add_argument("--id", required=True)
