...
price (self-reinforcing = fragile, crash-prone). When negative, leverage FADES price (mean-reverting,
...
def oi_hist(sym):
    d = _get(f"https://fapi.binance.com/futures/data/openInterestHist?symbol={sym}"
             f"&period={PERIOD}&limit={LIMIT}")
    return {int(x["timestamp"]): float(x["sumOpenInterest"]) for x in d} if isinstance(d, list) else {}


def klines(sym):
    d = _get(f"https://fapi.binance.com/fapi/v1/klines?symbol={sym}&interval={PERIOD}&limit={LIMIT}")
...
def main() -> None:
    ...
    for sym in SYMS:
        ...
        print(f"   -> forward VOL      IC {ic_v:+.4f} (t {t_v:+.2f}) | "
              f"high-refl vol {hi_v*100:.3f}% vs low {lo_v*100:.3f}%  ratio {hi_v/max(lo_v,1e-9):.2f}x")
        print(f"   -> forward DRAWDOWN IC {ic_d:+.4f} (t {t_d:+.2f}) | "
              f"high-refl dd {hi_d*100:+.3f}% vs low {lo_d*100:+.3f}%")
        print(f"   -> direction control (is it secretly a return signal?): "
              f"IC {scr.get('ic'):+.4f} {scr['verdict']}\n")
        out.append({"sym": sym, "n": n, "reflexivity_now": round(float(refl[-1]), 4),
                    "ic_vol": round(ic_v, 4), "t_vol": round(float(t_v), 2),
                    "ic_dd": round(ic_d, 4), "t_dd": round(float(t_d), 2),
                    "vol_ratio_hi_lo": round(float(hi_v / max(lo_v, 1e-9)), 3),
                    "dd_hi": round(float(hi_d), 5), "dd_lo": round(float(lo_d), 5),
    ...
    Path("data/reflexivity_m5.json").write_text(json.dumps(
