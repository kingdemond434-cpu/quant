...
def main() -> None:
    ...
    for sym, win, cond, _fam, _is_universe in blocked:
        ...
        st.update({
            ...
            "why": "no exact original-ten-gate certificate",
            "last_attempt_at": attempt_at,
        })
        state[key] = st
    for sym, win, cond, fam, is_universe in admitted:
        key = f"{sym}.{win}" + (f".{cond}" if cond else "")
    ...
    state["configured_sleeves"] = len(admitted)
    state["gate_blocked_sleeves"] = len(blocked)
