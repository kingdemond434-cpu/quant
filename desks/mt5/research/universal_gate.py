...
def main() -> int:
    ...
    # NON-DESTRUCTIVE MERGE (2026-08-26). This wrote `survivors_all` -- THIS RUN's passes --
    # straight over the authority file, so a sweep that certified nothing DELETED every
    # certificate the desk had ever earned. Measured: the 00:45 run took n=1 (the AUDNZD
    # hunt16 survivor, earned 08-23) to n=0 in one write, silently, with exit code 0.
    #
    # A certificate is EVIDENCE OF A PASSED GAUNTLET AT A POINT IN TIME. A later sweep that
    # does not re-derive it has not disproved it -- it has, at most, not re-tested it (L1.28a:
    # unmeasured is not a verdict). Existing entries are therefore carried forward and this
    # run's results are layered on top; a genuine failure must be recorded as a failure, by a
    # path that says so, never as an absence.
    _auth = REPORTS / "UNIVERSAL_SURVIVORS.json"
    try:
        _prior = json.loads(_auth.read_text(encoding="utf-8")).get("survivors", {})
    except (OSError, ValueError):
        _prior = {}
    if _prior and not survivors_all:
        print(f"universal_gate: this sweep certified 0 cells; PRESERVING {len(_prior)} existing "
              f"certificate(s) rather than erasing them -- re-running a gauntlet is not the same "
              f"as revoking a pass. Nothing written.")
        survivors_all = dict(_prior)
    else:
        merged = dict(_prior)
        merged.update(survivors_all)
        if len(merged) != len(survivors_all):
            print(f"universal_gate: merged {len(survivors_all)} fresh + "
                  f"{len(merged) - len(survivors_all)} carried-forward certificate(s)")
        survivors_all = merged
    _auth.write_text(
