...
def _replace(src: str, dst: Path) -> None:
    """os.replace, but survives a READ-ONLY destination on Windows.

    THE PORTED FIX WAS POSIX-ONLY AND THE BOX THAT TRADES IS WINDOWS. POSIX rename over a
    read-only file is legal -- permission lives on the DIRECTORY -- so `os.replace` was fine on
    the VPS and raised `PermissionError: [WinError 5]` here, on precisely the read-only snapshot
    case the atomic publish exists to handle. Silently keeping the POSIX form would have left the
    trading box unable to advance its crawl frontier at all: worse than the non-atomic write it
    replaced, because that one at least succeeded.

    So the attribute is cleared and the replace retried ONCE. A second failure is raised, never
    swallowed -- a frontier that cannot be published must say so rather than look saved.
    """
    try:
        os.replace(src, dst)
        return
    except PermissionError:
        if os.name != "nt" or not dst.exists():
            raise
