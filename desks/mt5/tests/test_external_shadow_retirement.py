...
def test_old_external_clock_is_retired_without_deleting_evidence(
    ...
    cert_path = tmp_path / "UNIVERSAL_SURVIVORS.json"
    ...
    cert_path.write_text(json.dumps({"survivors": {
        "external.XAUUSD.session_range_breakout": {
            "shadow_spec": {"params": {"rr": 2.0}},
        },
    }}), "utf-8")
    monkeypatch.setattr(external_shadow, "CERTS", cert_path)
...
def test_retirement_is_idempotent(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(external_shadow, "STATE", state_path)
    cert_path.write_text(json.dumps({"survivors": {}}), "utf-8")
    monkeypatch.setattr(external_shadow, "CERTS", cert_path)
    assert external_shadow.main() == 0
    assert external_shadow.main() == 0
    got = json.loads(state_path.read_text("utf-8"))
...
def test_guessed_default_clock_is_retired_as_unreconstructible(
    tmp_path: Path, monkeypatch,
) -> None:
    state_path = tmp_path / "external_shadow_state.json"
    cert_path = tmp_path / "UNIVERSAL_SURVIVORS.json"
    state_path.write_text(json.dumps({"external.X": {"status": "ACTIVE", "n": 0}}), "utf-8")
    cert_path.write_text(json.dumps({"survivors": {
        "external.X": {"shadow_spec": {"family": "session_range_breakout"}},
    }}), "utf-8")
    monkeypatch.setattr(external_shadow, "STATE", state_path)
    monkeypatch.setattr(external_shadow, "CERTS", cert_path)
    ...
    assert external_shadow.main() == 0
    got = json.loads(state_path.read_text("utf-8"))
    assert got["external.X"]["status"] == "RETIRED_UNRECONSTRUCTIBLE"
    assert got["retired_unreconstructible_sleeves"] == 1
