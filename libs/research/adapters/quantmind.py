"""quantmind adapter -- REJECTED_WITH_EVIDENCE for direct execution: a namesake on PyPI plus a
credential+LLM requirement; reading a namesake's terms is worse than UNVERIFIED. Every module
tried is named with its import result, so the refusal is a measurement rather than an absence.
Cover: libs/research/adapters/akshare.py + libs/research/axis_screen.py."""
...
TRIED: tuple[str, ...] = (
    "pip: the PyPI name resolves to an unrelated project (namesake risk)",
    "import: absent on this interpreter",
    "the stack is six credential-gated sources behind an LLM orchestrator",
)
WHY = ("a namesake on PyPI plus a credential+LLM requirement; reading a namesake's terms is "
    "worse than UNVERIFIED")
COVER = "libs/research/adapters/akshare.py + libs/research/axis_screen.py"
