"""openfr adapter -- REJECTED_WITH_EVIDENCE for direct execution: no package; the data tools are
AkShare calls already covered and the swarm needs an LLM. Every module tried is named with its
import result, so the refusal is a measurement rather than an absence. Cover:
libs/research/adapters/akshare.py."""
...
TRIED: tuple[str, ...] = (
    ...
    "35+ local data tools sit behind an LLM swarm",
)
WHY = ("no package; the data tools are AkShare calls already covered and the swarm needs an "
    "LLM")
COVER = "libs/research/adapters/akshare.py"
