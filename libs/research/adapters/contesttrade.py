"""contesttrade adapter -- REJECTED_WITH_EVIDENCE for direct execution: no package and an LLM
population; the competition itself is the donated idea. Every module tried is named with its
import result, so the refusal is a measurement rather than an absence. Cover:
libs/research/arena.py + libs/research/bandit.py."""
...
TRIED: tuple[str, ...] = (
    ...
    "the competitive population is prompted agents, not code carrying a mechanism",
)
WHY = "no package and an LLM population; the competition itself is the donated idea"
COVER = "libs/research/arena.py + libs/research/bandit.py"
