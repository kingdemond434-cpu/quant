...
SPECS: dict[str, Spec] = {s.system_id: s for s in (
    ...
    Spec("pyrqa", "PyRQA", "8.1.0", "pyrqa", yields=("representations",),
         note="sdist-only on PyPI and OpenCL-backed; measured 2026-09-23, `pip install "
              "--only-binary :all: PyRQA` finds NO distribution on CPython 3.14, so the pin "
              "exists to let provisioning attempt the sdist ONCE and settle the row"),
...
SPECS.update({s.system_id: s for s in (
    Spec("akshare", "akshare", "1.18.97", "akshare",
    ...
    Spec("ml_quant_trading", "", "", "ml_quant_trading",
         ...
         note="PROBE: no wheel resolved 2026-09-23"),
    Spec("qlib", "pyqlib", "0.9.6", "qlib",
         weight="heavy", yields=('representations',),
         note="PROBE: needs a provider_uri data directory the sandbox does not carry; "
              "`pip index versions pyqlib` returned NO versions on this interpreter "
              "(2026-09-23), so the pin exists to let provisioning settle the row once"),
    Spec("fingpt", "", "", "fingpt",
         weight="heavy", yields=('representations',),
