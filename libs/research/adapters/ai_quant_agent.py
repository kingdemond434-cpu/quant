"""ai_quant_agent adapter -- REJECTED_WITH_EVIDENCE for direct execution: no installable
distribution and an LLM+token requirement the research path refuses. Every module tried is named
with its import result, so the refusal is a measurement rather than an absence. Cover:
libs/research/adapters/akshare.py (the data surface) + the desk's own rule compiler."""
...
TRIED: tuple[str, ...] = (
    "pip: no distribution named ai-quant-agent or aiquantagent resolved on 2026-09-23",
    "import: neither module is present on this interpreter",
    "the hybrid needs an LLM key AND a Tushare/AkShare token; the sandbox carries neither",
)
WHY = "no installable distribution and an LLM+token requirement the research path refuses"
COVER = "libs/research/adapters/akshare.py (the data surface) + the desk's own rule compiler"
