"""vietnam_alpha adapter -- REJECTED_WITH_EVIDENCE for direct execution: no wheel and an LLM-driven
loop; the trajectory mechanism is rebuilt in numpy instead. Every module tried is named with its
import result, so the refusal is a measurement rather than an absence. Cover:
libs/research/adapters/quantaalpha.py."""
...
CANDIDATE_MODULES: tuple[str, ...] = (
    "vn_multi_agent_alpha",
    "vietnam_alpha",
)
TRIED: tuple[str, ...] = (
    "pip: no distribution resolved on 2026-09-23",
    "import: absent on this interpreter",
    "the self-improvement loop is an LLM rewriting prompts",
)
WHY = "no wheel and an LLM-driven loop; the trajectory mechanism is rebuilt in numpy instead"
COVER = "libs/research/adapters/quantaalpha.py"
