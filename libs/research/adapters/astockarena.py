"""astockarena adapter -- REJECTED_WITH_EVIDENCE for direct execution: no package, LLM-driven, and
its universe is the event lane by the two-lane law. Every module tried is named with its import
result, so the refusal is a measurement rather than an absence. Cover: libs/research/arena.py."""
...
TRIED: tuple[str, ...] = (
    ...
    "the arena is A-share single names driven by synchronised LLM agents",
)
WHY = "no package, LLM-driven, and its universe is the event lane by the two-lane law"
COVER = "libs/research/arena.py"
