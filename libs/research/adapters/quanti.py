"""quanti adapter -- REJECTED_WITH_EVIDENCE for direct execution: a namesake on PyPI and a service
rather than a library: nothing honest to import. Every module tried is named with its import
result, so the refusal is a measurement rather than an absence. Cover:
libs/research/adapters/alphacrafter.py (REBUILT regime selection, numpy)."""
...
TRIED: tuple[str, ...] = (
    "pip: the name resolves to an unrelated project (namesake risk)",
    "import: absent on this interpreter",
    "the platform is an A-share autonomous loop behind a web service",
)
WHY = "a namesake on PyPI and a service rather than a library: nothing honest to import"
COVER = "libs/research/adapters/alphacrafter.py (REBUILT regime selection, numpy)"
