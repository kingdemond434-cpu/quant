"""verumtrade adapter -- REJECTED_WITH_EVIDENCE for direct execution: no package; the EVIDENCE
GRAPH idea is carried by the desk's own causal organs. Every module tried is named with its
import result, so the refusal is a measurement rather than an absence. Cover:
libs/research/causal_graph.py + libs/research/artifact_chain.py."""
...
TRIED: tuple[str, ...] = (
    ...
    "the evidence graph and bull/bear reasoning are LLM prompts",
)
WHY = "no package; the EVIDENCE GRAPH idea is carried by the desk's own causal organs"
COVER = "libs/research/causal_graph.py + libs/research/artifact_chain.py"
