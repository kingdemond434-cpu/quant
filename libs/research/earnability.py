...
weight that earned it is wrong. Measured on BTCUSDT over 2,517 days the booked and earnable daily
funding panels have identical means to 8 decimal places and correlate 0.961 -- an estimator can be
exactly right on average and mis-paired on every single bar (the L1.47 shape, one layer up).
...
THE TWO CONVENTIONS, and the desk currently runs both:
  BOOKED    bins [t, t+1) -- pandas `resample("1D")` default, left-closed and left-labelled. Bar
            t is credited the settlements stamped 00:00, 08:00 and 16:00 OF DAY t.
  EARNABLE  bins (t, t+1] -- settlements strictly AFTER entry through exit inclusive. This is
            already the desk's law: `funding_clock.settlements_in` is documented half-open on
            exactly this window, because "a position closed exactly ON a stamp has already been
            paid". The research panel never got the memo.
...
the case for a rule that ranks on recent funding, since the settlement adjoining the signal window
carries more of the signal's own autocorrelation than the one a day further out.
...
Generalises to every DISCRETE flow: funding, maker rebates, staking, unlock cliffs, index
rebalances, airdrops. Anything paid at an instant rather than accrued over one.
"""
...
#: Phase conventions. BOOKED is what `crypto_source.bars_with_funding` produces today; EARNABLE is
#: what `funding_clock.settlements_in` already defines as the truth for a held position.
BOOKED = "booked"
EARNABLE = "earnable"
...
def normalise_stamps(ts: pd.DatetimeIndex, interval_h: float = fc.DEFAULT_INTERVAL_H
                     ) -> pd.DatetimeIndex:
    """Floor venue settlement stamps onto the settlement grid.

    Binance stamps `fundingTime` a few milliseconds PAST the boundary -- measured on BTCUSDT the
    offsets span 0-37ms across 38 distinct values. Those milliseconds are a bookkeeping artifact
    of the venue's settlement job, not an economic fact, and left unnormalised they scatter a
    ...
    Anchored to the UTC day, matching `funding_clock.last_settlement` (asserted in tests).
    """
    if interval_h <= 0:
...
def bin_flows(flows: pd.Series, *, phase: str, freq: str = "1D",
              interval_h: float = fc.DEFAULT_INTERVAL_H) -> pd.Series:
    ...
    EARNABLE -> bins (t, t+freq]  (strictly after entry through exit; `settlements_in` semantics)
...
def bin_panel(flows: dict[str, pd.Series] | pd.DataFrame, *, phase: str, freq: str = "1D",
              interval_h: float = fc.DEFAULT_INTERVAL_H) -> pd.DataFrame:
...
def attributable(flows: pd.Series, entry_ts: datetime, exit_ts: datetime, *,
    ...
    position could have been established, through the exit inclusive. That is
    `funding_clock.settlements_in`'s window, applied to values rather than counts.
...
def phase_sensitivity(build_returns: Callable[[pd.DataFrame], np.ndarray],
    ...
    `build_returns(funding_panel) -> np.ndarray` rebuilds the strategy's return series from a
    funding panel. `panels` maps each phase name to the panel binned under that convention. The
