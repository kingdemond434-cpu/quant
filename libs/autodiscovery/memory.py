...
class CandidateStore:
    ...
    def screen_survivors(self) -> list[CandidateRecord]:
        """Rows that cleared the statistical screen, including incomplete measurements.

        This is research inventory, not production authority. It preserves the historical meaning
        of the stored survived bit so trial accounting and retrospective audits never rewrite.
        """
        rows = self.db.execute(
    ...
    def survivors(self) -> list[CandidateRecord]:
        """Fully measured survivors only; lexical or incomplete rows cannot enter money paths.

        The stored survived bit is a Stage-A screen result. Two live rows carried survived=1 while
        their own rejection_reason said UNMEASURED sample adequacy, baselines and capacity, with
        capacity_usd=0. They appeared as dashboard survivors and then vanished from the promotion
        queue's capacity filter. This method is the authoritative boundary: every critical input
        must have been measured and positive capacity must exist. screen_survivors() retains the
        weaker historical population explicitly.
        """
        rows = self.db.execute(
            f"SELECT {_COLS} FROM research_candidates WHERE survived = 1 "
            f"AND status != '{CandidateStatus.ARCHIVED.value}' "
            "AND COALESCE(capacity_usd, 0) > 0 "
