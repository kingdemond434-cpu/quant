...
#: What a rung's entitlement is known to be. UNMEASURED is a FIRST-CLASS verdict and is never
#: collapsed into NOT-SERVED: "the vendor does not serve this id" and "we could not ask" are
#: different claims, and treating the second as the first is exactly the error R0361 was raised
#: about -- a reconnaissance pass reported claude-opus-5 nonexistent when it is served, and a
#: wrong edit here is self-reinforcing because is_upgrade ranks (5,) > (4,8).
SERVED = "SERVED"
NOT_SERVED = "NOT-SERVED"
UNMEASURED = "UNMEASURED"


def verify_chain(chain: list[str], served: list[str] | None) -> dict[str, object]:
    """Per-rung entitlement verdict for the LIVE chain, against the vendor's own listing.

    The desk could rank models and promote them but had no way to ask whether the rungs it
    already runs still answer. A dead middle rung is invisible: brain_auth_check walks past it,
    paying one CLI startup and round-trip per organ launch forever, and reports success from the
    rung below -- so the cost shows up as latency nobody attributes and never as a failure.

    `served=None` (or an empty listing, which means the call did not really succeed) yields
    UNMEASURED for every rung and a rollup of UNMEASURED. It must never yield NOT-SERVED: the
    caller cannot distinguish an unentitled id from an unreachable endpoint, and guessing is the
    failure this function exists to prevent.
    """
    have = set(served or ())
    measured = bool(have)
    rungs = [{"model": m,
              "status": (SERVED if m in have else NOT_SERVED) if measured else UNMEASURED,
              "rung": i}
             for i, m in enumerate(chain)]
    dead = [r["model"] for r in rungs if r["status"] == NOT_SERVED]
    rollup = UNMEASURED if not measured else ("DEAD-RUNG" if dead else "OK")
    return {
        "status": rollup,
        "rungs": rungs,
        "dead_rungs": dead,
        # L1.57: the denominator is what this RUN examined, never a hardcoded roster length.
        "n_rungs_checked": len(rungs) if measured else 0,
        "n_served_listed": len(have),
    }
