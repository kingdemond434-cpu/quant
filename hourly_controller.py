...
import numpy as np
import pandas as pd
# Import all components. Their canonical home is desks/mt5/side_channels/ -- the root-level
# duplicates were removed 2026-08-25 (newest copies synced there first), so the tracked home
...
class HourlySurvivorAcquisition:
    ...
    def _phase_discover(self) -> dict:
        ...
        if _side not in _sys.path:
            _sys.path.insert(0, _side)
        try:
            from run_all_miners import run_all_miners as _run_miners
            from convert_to_hypotheses import convert_discoveries as _convert
            miner_results = _run_miners()
            summary = miner_results.get("summary", {})
            hypotheses = _convert()
            discoveries = {}
            for name, data in miner_results.items():
            ...
            print(f"  Discovery: {total} raw discoveries, {len(hypotheses)} hypotheses ({elapsed:.1f}s)")
