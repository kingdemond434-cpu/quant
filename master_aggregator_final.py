#!/usr/bin/env python3
"""
FINAL MASTER SURVIVOR AGGREGATOR
================================
Properly extracts all fields from all sources for Claude delivery.
"""
    ...
    for name, s in web.get("sleeves", {}).items():
        if s.get("evidence") == "ACCRUING":
            ...
            if "::" in trial:
                ...
                if len(parts) >= 2:
                    s["family"] = parts[0]
                    s["subtype"] = parts[1] if len(parts) > 1 else ""
                    s["symbol"] = parts[2] if len(parts) > 2 else ""
            s["source"] = "paper_sleeve_forward"
            s["quality_score"] = s.get("progress_to_resolution", 0) * 100
            s["verdict"] = s.get("verdict", "ACCRUING")
            yield s
...
def extract_perpdex_survivors():
    ...
    if not doc:
        return
    axis = doc.get("axis", "")
    # mechanism_class is None for perpdex, infer from axis
    mechanism = "perpdex_funding"
    for cell in doc.get("screen_outputs", []):
        verdict = str(cell.get("verdict", ""))
        if verdict in ("SCREEN-INTERESTING", "SCREEN-WEAK"):
            # Build params from cell fields
            params = {
                "venue": cell.get("venue"),
                "resolution": cell.get("resolution"),
                "horizon_days": cell.get("horizon_days"),
                "n_eff": cell.get("n_eff"),
                "n": cell.get("n"),
                "decontam_passed": cell.get("decontam_passed"),
                "implausible_leak": cell.get("implausible_leak"),
                "current_z": cell.get("current_z"),
                "sharpe_momentum": cell.get("sharpe_momentum"),
                "sharpe_reversal": cell.get("sharpe_reversal"),
            }
            yield {
                **cell,
                "source": "perpdex_funding",
                "family": "perpdex_funding",
                "subtype": cell.get("name", "").split("::")[1] if "::" in cell.get("name", "") else "",
...
def extract_full_sweep_survivors_v2():
    """Extract from conv_full_sweep__survivors.json with proper field extraction."""
    path = _ROOT / "reports" / "axis_screens" / "conv_full_sweep__survivors.json"
    doc = load_json(_ROOT / path)
    if not doc:
        return
        ...
        if str(cell.get("verdict", "")) == "SCREEN-UNRATED":
            ...
            yield {
                **cell,
                "source": "full_sweep_survivors",
                "family": "full_sweep",
                "subtype": cell.get("name", ""),
                "symbol": cell.get("symbol", symbol),
                "mechanism": cell.get("mechanism", ""),
                "quality_score": 0,
                "verdict": "SCREEN-UNRATED",
            }
...
def main():
    ...
    print(f"Total unique survivors: {len(sorted_survivors)}")
    print(f"Claude delivery written")
    
    for i, s in enumerate(sorted_survivors[:15]):
