#!/usr/bin/env python3
"""Patch parse_screen_verdicts to add perpdex_funding schema handler"""

with open('/home/quant/quant-platform/libs/research/paper_sleeves.py', 'r') as f:
    content = f.read()

# Find the exact location to insert perpdex handler
# It's right after the first handler block ends (after `origin_key=str(doc.get("converted_key") or "trials")))`)
# and before `if not files_with_verdicts:`

old = '''                    origin_artifact=str(doc.get("converted_from") or f"{_AXIS_REL}/{p.name}"),
                    origin_key=str(doc.get("converted_key") or "trials")))

    if not files_with_verdicts:'''

new = '''                    origin_artifact=str(doc.get("converted_from") or f"{_AXIS_REL}/{p.name}"),
                    origin_key=str(doc.get("converted_key") or "trials")))
...
if old in content:
    ...
    with open('/home/quant/quant-platform/libs/research/paper_sleeves.py', 'w') as f:
        f.write(content)
    print("SUCCESS: perpdex_funding handler added to parse_screen_verdicts")
else:
    print("ERROR: Could not find target code")
    # Debug: find the area
    idx = content.find('origin_artifact=str(doc.get("converted_from") or f"{_AXIS_REL}/{p.name}"),')
    if idx >= 0:
        print("Found at:", idx)
